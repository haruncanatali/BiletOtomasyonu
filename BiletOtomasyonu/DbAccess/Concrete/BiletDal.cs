...
namespace BiletOtomasyonu.DbAccess.Concrete
{
    public class BiletDal:EntityRepositoryBase<Bilet,ProjeDbContext>,IBiletDal
    {
        public Bilet GetEntity(Expression<Func<Bilet, bool>> filter)
        {
            ...
            {
                return context.Tbl_Bilet.Include(c => c.Seferi).Include(c=>c.Sorumlusu).Include(c=>c.Musterisi).FirstOrDefault(filter);
            }
        }
        ...
        public List<Bilet> GetEntities(Expression<Func<Bilet, bool>> filter = null)
        {
            ...
            {
                return filter == null
                    ? context.Tbl_Bilet.Include(c => c.Seferi).Include(c => c.Sorumlusu).Include(c=>c.Musterisi).ToList()
                    : context.Tbl_Bilet.Include(c => c.Seferi).Include(c => c.Sorumlusu).Include(c=>c.Musterisi).Where(filter).ToList();
            }
        }
    }
}